...
<section><h2 class="sr-only">Ingredients</h2>{#each recipe.ingredient_groups as group}<section class="ingredients">{#if group.name}<h3>{group.name}</h3>{/if}{#each group.ingredients as row}<div class:checked={checked.includes(row.id)}><label class="ingredient"><input type="checkbox" checked={checked.includes(row.id)} onchange={() => checked = checked.includes(row.id) ? checked.filter(value => value !== row.id) : [...checked, row.id]}>{#if grams && hasGramAmount(row)}<span>{row.grams?.estimated === false ? '' : '≈ '}<Tooltip text={`As written: ${row.original_text || ingredientText(row)}`}><span class="gram-amount">{scaledAmount(row.grams!.amount!)} g</span></Tooltip>{' '}{gramTail(row)}</span>{:else}<span>{ingredientText(row, factor, grams)}</span>{/if}</label></div>{/each}</section>{/each}</section>
