<script lang="ts">
  ...
  let busy = $state(false), progress = $state(0), preview = $state(''), compressed = $state<File>(), needsName = $state(false), dragging = $state(false);
  ...
  async function upload() {
    ...
    try {const body = new FormData(); body.set('file', compressed); body.set('caption', caption); await mutate(`/recipes/${id(recipeId)}/photos`, body, 'POST', controller.signal, uploadKey); if (!alive) return; status = 'Photo submitted.'; compressed = undefined; URL.revokeObjectURL(preview); preview = ''; caption = ''; await reload();}
...
<div class="photo-grid">{#each photos as photo (photo.id)}<figure><a href={photoUrl(photo.id)} target="_blank" rel="noopener" aria-label="Open full-size photo"><img src={photoUrl(photo.id, true)} width="280" height="210" loading="lazy" alt={photo.caption || 'Recipe contributed by ' + (photo.uploader_name || 'a home cook')}></a><figcaption>{photo.caption}{#if photo.caption}<small>From {photo.uploader_name || 'a home cook'}</small>{/if}{#if photo.state !== 'approved'}<strong class="badge">Private submission: {photo.state}</strong>{/if}{#if photo.can_delete || photo.uploader_id === identity?.user?.id}<Button variant="ghost" size="sm" ariaLabel="Delete photo" disabled={busy} onclick={() => void removePhoto(photo)}><Icon name="trash" label="Delete photo" size={16} /></Button>{/if}</figcaption></figure>{/each}</div>
{#if !photos.length}<p class="quiet">No photos yet.</p>{/if}
{#if !mine}<section class="uploader"><h3>Add a photo</h3><label class:dragging class="dropper" ondragover={(event) => {event.preventDefault(); dragging = true;}} ondragleave={() => dragging = false} ondrop={dropPhoto}><input aria-label="Add a photo" type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onchange={(event) => void compress(event.currentTarget.files?.[0])}><Icon name="plus" size={18} />{busy ? 'Preparing photo…' : 'Drop a photo here or choose one'}<small>JPEG, PNG, or WebP</small></label>{#if busy}<p class="preparing"><Spinner label="Preparing photo" size={16} /> Preparing{progress ? ` ${Math.round(progress)}%` : '…'} <Button variant="ghost" size="sm" onclick={() => controller?.abort()}>Cancel</Button></p>{/if}{#if preview}<img class="photo-preview" src={preview} alt="Your contribution, ready to upload" width="320" height="240"><label>Caption<input maxlength="500" bind:value={caption}></label><Button variant="primary" disabled={busy} onclick={upload}>Submit photo</Button>{/if}{#if needsName && !identity?.user}<IdentityPrompt onready={(value) => {identity = value; needsName = false; status = 'Profile ready.';}} />{/if}</section>{/if}
<style>
  .quiet{color:var(--ui-text-muted)}.uploader{max-width:26rem;margin-top:1.25rem}.dropper{display:flex;flex-direction:column;align-items:center;gap:.3rem;padding:1rem;border:1px dashed var(--ui-control-border);border-radius:.55rem;background:var(--ui-surface-muted);cursor:pointer;text-align:center}.dropper.dragging{border-color:var(--ui-accent);background:var(--ui-surface)}.dropper input{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}.dropper small{color:var(--ui-text-muted)}.preparing{display:flex;align-items:center;gap:.4rem}.photo-preview{display:block;max-width:100%;height:auto;margin:.8rem 0}.uploader>label:not(.dropper){display:block;margin:.7rem 0}.uploader input:not([type=file]){display:block;width:100%;margin-top:.25rem}
</style>
